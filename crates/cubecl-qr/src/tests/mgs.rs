...
pub fn test_qr_mgs<R: Runtime, F: Float + CubeElement + Display>(device: &R::Device, dim: u32) {
    ...
    let (mut q, r) = match crate::launch::<R, F>(&crate::Strategy::ModifiedGramSchmidt, &client, &a)
    {
        Ok((q, r)) => (q, r),
        Err(_) => (
    ...
    cubecl_matmul::kernels::naive::launch::<R, F, F>(&client, q, r, &out.as_ref()).unwrap();
