...
#[cube(launch_unchecked)]
pub fn givens_rotation<F: Float>(
    col_index: u32,
    l: &Tensor<F>,
    q: &mut Tensor<F>,
    r: &mut Tensor<F>,
    ...
    let col_len = l.len();
    let mut j = col_len - 1;
    let mut mu_prime_i = hypot::<F>(l[j], l[j - 1]);
    let mut c = l[j - 1] / mu_prime_i;
    let mut s = l[j] / mu_prime_i;
...
pub fn launch<R: Runtime, E: Float + CubeElement>(
    ...
    for i in 0..r.shape[1] - 1 {
        unsafe {
            ...
            givens_rotation::launch_unchecked::<E, R>(
                client,
                cube_count.clone(),
                cube_dim,
                ScalarArg::new(i as u32),
